...
@Component({
  ...
  standalone: true,
  imports: [RouterLink, Button, Card, DatePipe, LoadingComponent],
  template: `
    <div class="dashboard">
      ...
      @if (loading()) {
        <app-loading label="Loading your dashboard..." />
      } @else {
  ...
  async ngOnInit(): Promise<void> {
    try {
      ...
      if (allSessions.length > 0) {
        this.lastWorkoutDate.set(allSessions[0].startTime.toLocaleDateString());
      }
    } finally {
